...
class JsonManager 
{
    ...
    public function removeAllEntries(): bool 
    {
        // Clear the data array
        $this->data = [];

        // Save the empty data back to the JSON file
        if ($this->saveData()) {
            return true; // Successfully cleared and saved
        }
...
function updateFirewallBlacklistIPs($Param_ip, $Param_FDIPset, $hostname, $cookie_path)
{
    $FDIPsetoutput = array();
    $FDIPsetstatus = "";
    $FDIPsetoutput = openWRTFirewallJSONtoArray($hostname, $cookie_path);
    ...
    else
    {
        ...
        {
            //echo "IPSet Name " . $Param_FDIPset . "</br>";
            if ($ipset['name'] === $Param_FDIPset)
            {   
                // Check if is an array
                if (is_array($Param_ip)) 
                {
                    //echo "String of MAC " . $addMAC . "</br>";
                    $JSON_data_add = json_encode(array( "method" => "set", "params" => array("firewall", $ipset['.name'], "dest_ip", $Param_ip)));
                    $response_info = PostOpenWRTAPI("https://" . $hostname . "/cgi-bin/luci/rpc/uci", $JSON_data_add, $cookie_path);
